...
struct UltrasonicScalerDetailView: View {
    let userId: Int
    ...
    private func loadProduct() {
        guard let url = URL(string: "\(ServiceAPI.productDetails)?product_id=\(7)") else {
    ...
    private func addToCart() {
        guard let product = product,
            ...
            errorMessage = "Invalid Add to Cart request"
            showErrorAlert = true
            return
        }
...
struct UltrasonicScalerDetailView_Previews: PreviewProvider {
    static var previews: some View {
        UltrasonicScalerDetailView(userId: 1)
    }
}
