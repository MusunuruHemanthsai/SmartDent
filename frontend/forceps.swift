...
struct ForcepsProductResponse: Codable {
    let status: String
    let data: ForcepsProduct
}

// MARK: - View
struct ForcepsDetailView: View {
    @AppStorage("user_id") private var userId: Int = 0

    @State private var product: ForcepsProduct?
    ...
    private func loadProduct() {
        guard let url = URL(string: "\(ServiceAPI.productDetails)?product_id=\(21)") else {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            DispatchQueue.main.async {
                ...
                do {
                    let decoded = try JSONDecoder().decode(ForcepsProductResponse.self, from: data)
...
struct ForcepsDetailView_Previews: PreviewProvider {
