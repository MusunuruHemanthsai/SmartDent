...
struct BoneGraftingDetailView: View {
    ...
    private func fetchProduct() {
        guard let url = URL(string: "\(ServiceAPI.productDetails)?product_id=\(25)") else {
                    ...
                    if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                       let status = json["status"] as? String {
                        if status == "success" {
                            ...
                            errorMessage = json["message"] as? String ?? "Failed to add item"
                            showErrorAlert = true
                        }
                    } else {
